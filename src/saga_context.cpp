...
const EventBook* SagaContext::get_destination(
    const std::string& domain, const std::string& aggregate_root) const {
  std::string key = make_key(domain, aggregate_root);
  auto it = destinations_.find(key);
...
std::string SagaContext::make_key(const std::string& domain,
