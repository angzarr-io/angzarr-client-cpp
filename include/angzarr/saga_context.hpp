 ...
 * aggregates. Provides sequence number lookup for optimistic concurrency
 * control.
 *
 * Example usage:
 * @code
...
class [[deprecated("Use Destinations instead")]] SagaContext {
 public:
   ...
   */
  uint32_t get_sequence(const std::string& domain,
   ...
   */
  bool has_destination(const std::string& domain,
                       const std::string& aggregate_root) const;

 private:
  void add_destination(const EventBook& book);
  static std::string make_key(const std::string& domain,
