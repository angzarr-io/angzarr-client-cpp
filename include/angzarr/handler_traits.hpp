...
// ============================================================================
// Projection (key-value pair returned by projector handlers)
// ============================================================================
//
...
struct Projection {
  ...
  static Projection upsert(const std::string& k, const std::string& v) {
    return {k, v, false};
  }
  static Projection remove(const std::string& k) { return {k, "", true}; }
};
...
struct ProcessManagerResponse {
  ...
  /// Create response with commands only.
  static ProcessManagerResponse with_commands(
      std::vector<CommandBook> commands) {
  ...
  static ProcessManagerResponse with_both(std::vector<CommandBook> commands,
                                          EventBook events) {
    return {std::move(commands), std::move(events), {}};
  }

  /// Create response with all fields.
  static ProcessManagerResponse with_all(std::vector<CommandBook> commands,
                                         EventBook events,
                                         std::vector<EventBook> facts) {
 ...
 *
 * Provides a builder pattern for registering event appliers with
 * auto-unpacking. Register once at startup, call `with_event_book()` per
 * rebuild.
 *
...
class CommandHandlerDomainHandler {
 public:
   ...
   */
  virtual EventBook handle(const CommandBook& cmd,
                           const google::protobuf::Any& payload, const S& state,
                           int seq) = 0;
   ...
   * @return RejectionHandlerResponse with compensation events or forwarded
   * notification
   */
  virtual RejectionHandlerResponse on_rejected(
      const Notification& notification, const S& state,
...
class SagaDomainHandler {
 public:
   ...
   */
  virtual SagaHandlerResponse execute(const EventBook& source,
                                      const google::protobuf::Any& event,
   ...
   */
  virtual RejectionHandlerResponse on_rejected(
      const Notification& notification, const std::string& target_domain,
...
class ProcessManagerDomainHandler {
 public:
   ...
   */
  virtual ProcessManagerResponse handle(const EventBook& trigger,
                                        const S& state,
                                        const google::protobuf::Any& event,
                                        const Destinations& destinations) = 0;
...
class ProjectorDomainHandler {
 public:
  /// Optional catch-all hook — invoked with the event's type_url
  /// when no declared ``event_types()`` arm matches. Mirrors Rust's
  /// ``#[handles_unknown]`` (commit 804c362). User code uses it to
  /// render a placeholder, increment a counter, or anything else
  /// that needs the unmatched type URL.
  using UnknownHandler = std::function<void(const std::string& type_url)>;
  ...
  /// Register (or clear) the catch-all hook. Cleared by passing an
  /// empty ``std::function``. Mirrors Rust's ``#[handles_unknown]``
  /// attribute (804c362) which annotated a single method on the
  /// projector struct; the C++ port uses an explicit setter since
  /// there's no proc-macro layer.
  void set_unknown_handler(UnknownHandler handler) {
    unknown_handler_ = std::move(handler);
  }

  /// Invoke the registered catch-all, if any. Used by
  /// ``ProjectorRouter::dispatch_or_warn`` after emitting the
  /// canonical WARN log line.
  void dispatch_unknown(const std::string& type_url) const {
    if (unknown_handler_) unknown_handler_(type_url);
  }

  /// ``true`` iff a catch-all has been registered.
  bool has_unknown_handler() const {
    return static_cast<bool>(unknown_handler_);
  }

 private:
  UnknownHandler unknown_handler_;
};
