...
#include "errors.hpp"
#include "handler_traits.hpp"  // for Projection
#include "helpers.hpp"
#include "router.hpp"

namespace angzarr {

// ``Projection`` is defined in handler_traits.hpp so
// ProjectorDomainHandler can reference it without a circular include
// from projector.hpp.
 ...
 *           return Projection::upsert(event.sku(),
...
class Projector {
 public:
  using ProjectionHandler =
      std::function<Projection(Projector*, const google::protobuf::Any&)>;
 ...
 protected:
   ...
   */
  static void register_projector_handler(const std::string& suffix,
