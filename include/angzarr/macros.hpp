 ...
 *
 * RECOMMENDED: Use the CRTP-based CommandHandlerBase with constructor
 * registration instead of these macros. The CRTP pattern is type-safe and
 * similar to Go:
 *
 * @code
 ...
 * They require passing the class name explicitly to avoid `decltype(this)`
 * issues.
 */
...
#define ANGZARR_HANDLES_FOR(Class, T)                                          \
  ...
  static ::angzarr::EventBook _dispatch_##T(                                   \
      Self* self, const ::google::protobuf::Any& any, int seq) {               \
    T cmd;                                                                     \
    any.UnpackTo(&cmd);                                                        \
...
#define ANGZARR_APPLIES_FOR(Class, T)                                       \
  ...
  static void _apply_##T(Self* self, State& state,                          \
...
#define ANGZARR_REJECTED_FOR(Class, target_domain, command_type)           \
  static inline const bool _rejected_##command_type##_registered =         \
      (register_rejection_handler(target_domain                            \
                                  "/" ANGZARR_TYPE_SUFFIX(command_type),   \
...
#define ANGZARR_REJECTED(domain, \
                         cmd)  // BROKEN: uses decltype(this) at class scope
