 ...
 */
inline void require_exists(
    bool exists, const std::string& message = "Aggregate does not exist") {
  if (!exists) {
 ...
 */
inline void require_not_exists(
    bool exists, const std::string& message = "Aggregate already exists") {
  if (exists) {
    throw CommandRejectedError(message);
  }
}
 ...
 */
inline void require_not_empty(const std::string& value,
 ...
 */
template <typename T>
void require_status(T actual, T expected,
                    const std::string& message = "Invalid status") {
  if (actual != expected) {
    throw CommandRejectedError(message);
  }
}
 ...
 */
template <typename T>
void require_status_not(T actual, T forbidden,
                        const std::string& message = "Invalid status") {
  if (actual == forbidden) {
    throw CommandRejectedError(message);
  }
