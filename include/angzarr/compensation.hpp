 ...
 */
constexpr const char* kNotificationTypeName = "angzarr.Notification";
...
/**
 * Holds PM compensation results: optional process events plus framework action flags.
 */
 ...
 */
inline PMRevocationResponse pm_emit_compensation_events(const EventBook& events,
                                                        bool also_emit,
...
inline bool is_notification(const std::string& type_url) {
    return type_url == std::string(helpers::TYPE_URL_PREFIX) + kNotificationTypeName;
}
