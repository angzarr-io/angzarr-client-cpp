 ...
 *           handles_rejection("table", "JoinTable",
 * &Player::handle_join_rejected);
 *       }
 *
 ...
 * event); void apply_deposited(PlayerState& state, const FundsDeposited&
 * event);
 *
...
class CommandHandlerBase {
 public:
  ...
  using CommandDispatcher =
      std::function<EventBook(Derived*, const google::protobuf::Any&, int)>;
  using EventApplier =
      std::function<void(Derived*, StateT&, const google::protobuf::Any&)>;
  using RejectionDispatcher =
      std::function<EventBook(Derived*, const Notification&)>;
  ...
  Derived& handles(Event (Derived::*method)(const Command&)) {
    ...
    handlers_[suffix] = [method](Derived* self,
                                 const google::protobuf::Any& any, int seq) {
      Command cmd;
      any.UnpackTo(&cmd);
  ...
  Derived& applies(void (Derived::*method)(StateT&, const Event&)) {
    ...
    appliers_[suffix] = [method](Derived* self, StateT& state,
   ...
   */
  Derived& handles_rejection(
      const std::string& source_domain, const std::string& command_type,
      EventBook (Derived::*method)(const Notification&)) {
    auto key = source_domain + "/" + command_type;
    rejection_handlers_[key] = [method](Derived* self,
...
/**
 * Legacy base class for command handlers using macro-based handler
 * registration.
...
class CommandHandler {
 public:
  using State = StateT;
  using CommandDispatcher = std::function<EventBook(
      CommandHandler*, const google::protobuf::Any&, int)>;
  using EventApplier = std::function<void(CommandHandler*, StateT&,
 ...
 protected:
  ...
  static void register_handler(const std::string& suffix,
                               CommandDispatcher dispatcher) {
    handlers()[suffix] = std::move(dispatcher);
  }

  static void register_applier(const std::string& suffix,
                               EventApplier applier) {
    appliers()[suffix] = std::move(applier);
  }

  static void register_rejection_handler(const std::string& key,
