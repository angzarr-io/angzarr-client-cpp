...
class CommandRouter {
 public:
  ...
  using RejectionHandler =
      std::function<RejectionHandlerResponse(const Notification&, State&)>;
  using StateRebuilder = std::function<State(const EventBook*)>;

  explicit CommandRouter(const std::string& domain,
                         StateRebuilder rebuild = nullptr)
   ...
   * This provides type-safe auto-unpacking of commands and auto-packing of
   * events.
   *
   * @tparam Command The protobuf command type
   * @tparam Event The protobuf event type to emit
   * @param handler Function taking (const Command&, const State&) and returning
   * Event
   */
  template <typename Command, typename Event>
  CommandRouter& on(
      std::function<Event(const Command&, const State&)> handler) {
    std::string suffix = Command::descriptor()->full_name();
    handlers_.emplace_back(
        suffix,
        [handler](const CommandBook& cmd_book,
                  const google::protobuf::Any& cmd_any, State& state,
                  int seq) -> EventBook {
          Command cmd;
          // Spec MED-4.10 — wrap UnpackTo so a bad payload raises
          // a structured ``ANY_DECODE_FAILED`` instead of
          // silently producing a default-initialized Command.
          if (!cmd_any.UnpackTo(&cmd)) {
            throw InvalidArgumentError(
                error_codes::messages::ANY_DECODE_FAILED,
                error_codes::codes::ANY_DECODE_FAILED,
                ClientError::DetailsMap{
                    {error_codes::keys::TYPE_URL, cmd_any.type_url()},
                });
          }
   ...
   */
  CommandRouter& on_rejected(const std::string& domain,
 ...
 private:
  ...
  BusinessResponse dispatch_rejection(const Notification& notification,
    ...
    for (const auto& [key, handler] : rejection_handlers_) {
      ...
      if (domain == expected_domain && simple_command == expected_command) {
        ...
        if (response.notification.has_value()) {
          ...
          *biz_response.mutable_notification() =
              std::move(*response.notification);
          return biz_response;
        }
        ...
        revocation->set_reason("Aggregate " + domain_ +
                               " handled rejection for " + key);
        return biz_response;
      }
    }
    ...
    revocation->set_reason("Aggregate " + domain_ +
                           " has no custom compensation for " + domain + "/" +
                           command_suffix);
    return response;
  }
...
class EventRouter {
 public:
   ...
   */
  std::vector<CommandBook> dispatch(
      const EventBook& book, const std::vector<EventBook>& destinations = {}) {
    std::string source_domain;
    if (book.has_cover()) {
      source_domain = book.cover().domain();
    }
    ...
    for (const auto& page : book.pages()) {
      ...
      for (const auto& [suffix, handler] : it->second) {
        if (helpers::type_url_matches(page.event().type_url(), suffix)) {
          auto cmds =
              handler(page.event(), root_str, correlation_id, destinations);
 ...
 private:
  ...
  std::map<std::string, std::vector<std::pair<std::string, EventHandler>>>
      handlers_;
  std::map<std::string, std::map<std::string, PrepareHandler>>
      prepare_handlers_;
};
